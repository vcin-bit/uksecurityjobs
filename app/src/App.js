...
function StepPhoto({ data, onChange, onBack, onNext }) {
  const [preview, setPreview] = useState(data.photo?.preview || null);
  const [uploaded, setUploaded] = useState(data.photo?.uploaded || false);
  ...
  return (
    <StepShell step={8} total={11} title="Profile Photo"
      ...
      onBack={onBack} onNext={save} nextLabel={uploaded ? 'Save & Continue' : 'Skip for now'}>
